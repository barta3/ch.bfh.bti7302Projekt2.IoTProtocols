...
public class MqttClient implements IMqttClient { //), DestinationProvider {
	...
	public void publishWithSchema(String topic, MqttMessageJson<?,?> messageJson, boolean pubState) throws MqttException,
	MqttPersistenceException {
		
		// Convert
		MqttMessage message = new MqttMessage();
		message.setPayload(getData(messageJson.getPayload()));
		message.setRetained(messageJson.isRetained());
		message.setQos(messageJson.getQos());
		
		// Message
		aClient.publish(topic, message, null, null).waitForCompletion(getTimeToWait());
		
		boolean fist = schemaCount++ ==0;
		if(fist) {
			// Schema
			message.setPayload(getSchema(messageJson.getPayload().getClass()).getBytes());
			message.setRetained(true);
			aClient.publish(topic + "/schema", message, null, null).waitForCompletion(getTimeToWait());
			
		}
		
		if(fist || pubState) {
			// Status
			MqttMessage msgState = new MqttMessage(getData(messageJson.getStatus()));
			msgState.setRetained(true);
			aClient.publish(topic + "/state/get", msgState, null, null).waitForCompletion(getTimeToWait());
		}
	}
	
	public void publishStatus(String topic, MqttMessageJson<?,?> messageJson) throws MqttException,
	MqttPersistenceException {
	...
	private byte[] getData(Object object) {
		try {
	...
	private String getSchema(Class cl) {
//		return "{\"$schema\":\"http://json-schema.org/draft-04/schema#\",\"id\":\"http://jsonschema.net\",\"type\":\"object\",\"properties\":{\"id\":{\"id\":\"http://jsonschema.net/id\",\"type\":\"string\"},\"value\":{\"id\":\"http://jsonschema.net/value\",\"type\":\"integer\"},\"timestamp\":{\"id\":\"http://jsonschema.net/timestamp\",\"type\":\"string\"}},\"required\":[\"id\",\"value\",\"timestamp\"]}";
		...
		String schema = "ERROR";
		try {
			mapper.acceptJsonFormatVisitor(mapper.constructType(cl), visitor);
